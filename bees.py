...
class BeesProblem(Individual):
    ...
    def mutate(self, mutate_params):
        ...
        board = BeesBoard.create(self.value['solution'],
                                 max_pips=max_pips)
...
class FitnessCalculator:
    ...
    def calculate(self, problem):
        ...
        if fitness is not None:
            return fitness
        board = BeesBoard.create(value['solution'], max_pips=value['max_pips'])
        graph = BeesGraph()
        fitness = 0
        try:
...
class BeesBoard(Board):
    ...
    def place_dice(self):
        ...
        for pips in range(1, self.max_pips + 1):
            for x in range(self.width):
                for y in range(self.height):
                    cell = self[x][y]
                    partner_cell = cell.partner
                    if partner_cell.pips == 0 and cell.pips != 0:
...
def main():
    ...
    for i in range(n_epochs):
        ...
        print(i, top_fitness, mid_fitness, repr(top_individual.value['solution']))
